...
\begin{tabularx}{\linewidth}{>{\RaggedRight\arraybackslash}p{3.2cm} >{\RaggedRight\arraybackslash}X >{\RaggedRight\arraybackslash}p{4.0cm} >{\RaggedRight\arraybackslash}p{4.0cm}}
\toprule
Tipo de modelo & Escopo & Mínimo viável & Nível ótimo \\
\midrule
\makecell[l]{Physic-based\\(balanços)} & \makecell[l]{Corridas +\\Informações Diária} & \makecell[l]{6–12 meses contínuos com\\timestamp padronizado} & \makecell[l]{18–24 meses com\\calibração documentada} \\\\
...
\makecell[l]{Não supervisionado\\(detecção de padrão)} & \makecell[l]{Supervisório (F4/F5);\\senão Consumo Fornos} & \makecell[l]{2–3 meses contínuos com\\timestamp confiável} & \makecell[l]{6–12 meses com tags\\harmonizadas e filtros de ruído} \\\\
\makecell[l]{Reforço (RL)\\(políticas)} & \makecell[l]{Supervisório com ações e\\retorno (F4/F5)} & \makecell[l]{3–6 meses de episódios com\\ação/recompensa registrada} & \makecell[l]{12+ meses com atrasos\\medidos e segurança validada} \\\\
\bottomrule
\end{tabularx}
\end{small}
...
\clearpage
\includepdf[pages=1]{/home/wilson/Maringa/quadro_consolidado_A3_landscape.pdf}
\clearpage
...
Regras de cálculo (qualitativas): - Classificação por domínio/forno:
\textbf{Nível 1} = utilizável com baixo risco (ajustes pontuais;
validade/consistência altas e timestamp padronizável). \textbf{Nível 2}
= utilizável com risco moderado (estrutura boa, mas exige padronização,
fechamento de lacunas e formalização de chave/tempo). \textbf{Nível 3} =
alto risco (problemas de parsing/estrutura, duplicidades ou completude
muito baixa; uso direto tende a gerar conclusões equivocadas). -
Dependências por tipo de modelo: \textbf{Physic-based} = Corridas +
Informações Diária; \textbf{Supervisionado} = Corridas + Informações
Diária + Consumo Fornos; \textbf{Não supervisionado} = Supervisório (se
existir) senão Consumo Fornos; \textbf{Reforço (RL)} = Supervisório
obrigatório. - Aplicação aos domínios (qualitativa): Consumo Fornos =
N3; Corridas = N2; Informações Diária = N2; Eletrodo = N2; Supervisório
F4 = N2; Supervisório F5 = N3. O nível por forno/modelo é o pior
(máximo) entre os domínios requeridos.
