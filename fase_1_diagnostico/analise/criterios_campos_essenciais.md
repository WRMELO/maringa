...
|-----------|----------|-------------------|----------|
| `chave_relacional` | Criar chaves para ligar tabelas (Forno, Corrida, Produto, Folha, Lote, Silo, Bica, Turno). | - Nome curto, sem hierarquia (sem `.`) e até 40 caracteres.- Contém palavras-chave: `forno`, `corrida`, `codigo`, `código`, `produto`, `lote`, `bica`, `silo`, `folha`, `turno`. | `Forno`, `Codigo`, `Lote_Destino`, `Silo` |
| `timestamp` | Garantir ordenação temporal e cálculo de cobertura. | - Tipo declarado `Data/hora` ou `Data Hora`.- ou nome contém `data` / `hora`. | `Data`, `Data_Base`, `Data_Final`, `Timestamp` |
